...
// Calls a backend endpoint. Adds the session token automatically if available.
// Defaults to POST. Use method 'GET' for read endpoints.
...
// All database access routes through Vercel serverless functions.
// This avoids browser-to-Supabase calls being blocked by school network filters.
const storage = {
  async list() {
    const result = await callApi('/api/list-tools', undefined, 'GET');
    return result.tools || [];
  },
...
// Public submission and admin management of evaluation requests
const requests = {
  async submit(payload) {
    return await callApi('/api/submit-request', payload);
  },
  async list() {
    const result = await callApi('/api/list-requests', undefined, 'GET');
    return result.requests || [];
  },
  async delete(id) {
    return await callApi('/api/delete-request', { id });
  }
};
...
// Ensures a URL has a protocol so it opens externally rather than as a site-relative path
...
export default function App() {
  ...
  async function saveTool(tool) {
    ...
    if (toolData.url) toolData.url = normaliseUrl(toolData.url);
    try {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? (
          ...
          <Dashboard
            ...
            onViewRequests={() => requireAuth(() => setView('requests'))}
          />
        ) : view === 'evaluate' ? (
          <EvaluationForm
            tool={currentTool}
          ...
          <RequestForm onCancel={() => setView('dashboard')} onSubmitted={() => setView('dashboard')} />
        ) : view === 'requests' ? (
          <RequestsList
            onBack={() => setView('dashboard')}
            onStartEvaluation={(prefilledUrl) => {
              setCurrentTool({
                ...emptyTool,
                url: prefilledUrl || '',
                tags: [],
...
function Dashboard({ tools, allTools, classCounts, searchQuery, setSearchQuery, filterClass, setFilterClass, filterTag, setFilterTag, filterAudience, setFilterAudience, hasAnyFilter, clearFilters, currentPage, setCurrentPage, onNew, onView, onEdit, isUnlocked, onLoginClick, onRequest, onViewRequests }) {
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
        <div className="flex flex-col gap-3">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-end gap-2">
            <button
              onClick={onRequest}
              className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg border font-medium text-sm transition hover:bg-pink-50 w-full sm:w-auto"
              style={{ borderColor: '#A6174A', color: '#A6174A', background: 'white' }}
            >
              <Plus className="w-4 h-4" /> Request Evaluation
            </button>
            {isUnlocked && (
              <>
                <button
                  onClick={onViewRequests}
                  className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-lg border border-gray-200 text-gray-700 font-medium text-sm transition hover:bg-gray-50 w-full sm:w-auto"
                >
      ...
      <div className="hidden lg:grid grid-cols-12 gap-4 px-5 py-4 items-start">
        <div className="col-span-2 min-w-0 flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            {tool.url && (
              <a href={normaliseUrl(tool.url)} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="flex items-center gap-1 text-xs truncate" style={{ color: '#A6174A' }}>
        ...
        <div className="p-6">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-4">
            <div className="min-w-0">
              ...
              {tool.url && (
                <a href={normaliseUrl(tool.url)} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm" style={{ color: '#A6174A' }}>
  ...
  if (success) {
    return (
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 p-8 text-center">
        ...
        <PinkHeading as="h2" size="lg" className="mb-2">Request submitted</PinkHeading>
        <p className="text-sm text-gray-600 mb-6 max-w-md mx-auto">
          Thanks for your request. The e-Learning team will review it and add an evaluation to the library when it has been assessed.
        </p>
        <button
          onClick={onSubmitted}
          className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg text-white font-medium text-sm transition hover:opacity-90"
          style={{ background: '#A6174A' }}
        >
    ...
    <div className="max-w-2xl mx-auto">
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div className="px-6 py-5 border-b border-gray-100">
          <PinkHeading as="h1" size="lg">Request an AI Tool Evaluation</PinkHeading>
          <p className="text-sm text-gray-600 mt-2">
            Found an AI tool you would like to use? Fill out this form and the e-Learning team will assess it for safety, ethics, and effectiveness.
          </p>
        </div>
        <div className="p-6 space-y-4">
          ...
          <Field label="Year Level / Department">
            <input
              ...
              placeholder="e.g., Year 9 English, Primary Maths, Library"
              className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:border-pink-400 text-sm"
            />
          </Field>
          <Field label="Intended Use" required>
            <textarea
              ...
              placeholder="What would you use this tool for? Who would use it (you, students, or both)?"
              rows={4}
              className="w-full px-3 py-2 rounded-lg border border-gray-200 focus:outline-none focus:border-pink-400 text-sm resize-none"
            />
          </Field>
          <Field label="Urgency">
            <div className="grid grid-cols-3 gap-2">
              {['Low', 'Standard', 'High'].map(opt => (
                <button
                  key={opt}
                  type="button"
                  onClick={() => setUrgency(opt)}
                  className={`px-3 py-2 rounded-lg border text-xs font-medium transition ${urgency === opt ? 'border-pink-400 bg-pink-50 text-pink-700' : 'border-gray-200 text-gray-600 hover:border-gray-300'}`}
                >
                  {opt}
                </button>
              ))}
            </div>
            <p className="text-xs text-gray-500 mt-1.5">High urgency is for tools needed within the next week or two.</p>
          </Field>
          ...
          <div className="flex gap-2 pt-2">
            ...
            <button
              ...
              disabled={submitting}
              className="flex-1 px-4 py-2.5 rounded-lg text-white text-sm font-medium transition hover:opacity-90 disabled:opacity-60"
              style={{ background: '#A6174A' }}
            >
              {submitting ? (
...
function RequestsList({ onBack, onStartEvaluation }) {
  ...
  async function handleDismiss(id) {
    if (!confirm('Dismiss this request? It will be removed from the queue.')) return;
    setDeletingId(id);
    try {
      await requests.delete(id);
      await load();
    } catch (e) {
      alert('Could not delete request: ' + (e.message || 'Unknown error'));
    } finally {
      setDeletingId(null);
    }
  }
  ...
  const URGENCY_COLOURS = {
    ...
    'Standard': { bg: '#DBEAFE', text: '#1E3A8A' },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <PinkHeading as="h1" size="xl">Evaluation Requests</PinkHeading>
          <p className="text-sm text-gray-600 mt-1">
            {loading ? 'Loading...' : `${items.length} request${items.length === 1 ? '' : 's'} in the queue`}
          </p>
        </div>
        <button
          onClick={onBack}
      ...
      {loading ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center text-gray-500">
          <Loader2 className="w-5 h-5 animate-spin inline-block mr-2" /> Loading requests...
        </div>
      ) : error ? (
        <div className="bg-white rounded-xl shadow-sm border border-red-200 p-8 text-center">
          <AlertCircle className="w-10 h-10 mx-auto mb-3 text-red-500" />
          <h3 className="text-lg font-semibold text-gray-900 mb-2">Could not load requests</h3>
          <p className="text-sm text-gray-600 mb-4">{error}</p>
          <button onClick={load} className="px-4 py-2 rounded-lg text-white text-sm font-medium hover:opacity-90 transition" style={{ background: '#A6174A' }}>Try again</button>
        </div>
      ) : items.length === 0 ? (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
          <FileText className="w-10 h-10 mx-auto mb-3 text-gray-300" />
          <h3 className="text-lg font-semibold text-gray-900 mb-2">No requests yet</h3>
          <p className="text-sm text-gray-500">When staff submit evaluation requests, they will appear here.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {items.map(item => {
            const r = item.data || {};
            const urgencyColour = URGENCY_COLOURS[r.urgency] || URGENCY_COLOURS.Standard;
                ...
                <div className="p-5">
                  <div className="flex items-start justify-between gap-3 flex-wrap mb-3">
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: urgencyColour.bg, color: urgencyColour.text }}>
                          {r.urgency || 'Standard'}
                        </span>
                        <span className="text-xs text-gray-500">{date}</span>
                      </div>
                      <a
                        href={normaliseUrl(r.toolUrl)}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="font-semibold text-base inline-flex items-center gap-1.5"
                        style={{ color: '#A6174A' }}
                      >
                        <ExternalLink className="w-3.5 h-3.5 flex-shrink-0" />
                        <span className="break-all">{(r.toolUrl || '').replace(/^https?:\/\//, '')}</span>
                      </a>
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-sm text-gray-900">{r.yearLevel || <span className="text-gray-400">Not specified</span>}</div>
                    </div>
                  </div>
                  <div className="mb-3">
                    <div className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-0.5">Intended Use</div>
                    <div className="text-sm text-gray-700 whitespace-pre-wrap">{r.intendedUse}</div>
                  </div>
                  {r.notes && (
                    <div className="mb-3">
                      <div className="text-xs font-semibold uppercase tracking-wide text-gray-500 mb-0.5">Additional Notes</div>
                      <div className="text-sm text-gray-700 whitespace-pre-wrap">{r.notes}</div>
                    </div>
                  )}
                  <div className="flex gap-2 pt-3 border-t border-gray-100">
                    <button
                      onClick={() => onStartEvaluation(r.toolUrl)}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-white text-sm font-medium transition hover:opacity-90"
                      style={{ background: '#A6174A' }}
                    >
                      <Sparkles className="w-3.5 h-3.5" /> Start Evaluation
                    </button>
                    <button
                      onClick={() => handleDismiss(item.id)}
                      disabled={deletingId === item.id}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg border border-gray-200 text-gray-600 text-sm hover:bg-gray-50 transition disabled:opacity-60"
                    >
